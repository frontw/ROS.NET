...
namespace FauxMessages
{
    public class SrvsFile
    {
        ...
        public void Write(string outdir)
        {
            ...
            if (contents != null)
				File.WriteAllText(outdir + Path.DirectorySeparatorChar + msgfilelocation.basename + ".cs", contents.Replace("FauxMessages", "Messages"));
        }
...
{0}ret &= {1}.Equals(other.{1});", leadingWhitespace, name);
        ...
        public void Write(string outdir)
        {
            ...
            if (serviceMessageType == ServiceMessageType.Response)
				File.AppendAllText(outdir + Path.DirectorySeparatorChar + localcn + ".cs", contents.Replace("FauxMessages", "Messages"));
            else
				File.WriteAllText(outdir + Path.DirectorySeparatorChar + localcn + ".cs", contents.Replace("FauxMessages", "Messages"));
        }
    }
